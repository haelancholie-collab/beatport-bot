...
def fetch_beatport_client_id():
    ...
    try:
        ...
        response = requests.get(f"{BEATPORT_API_BASE}/docs/")
        ...
        for js_url in js_urls:
            js_response = requests.get(f"{BEATPORT_API_BASE}{js_url}")
            match = re.search(r"API_CLIENT_ID: '([^']+)'", js_response.text)
            if match:
                client_id = match.group(1)
                print(f"Client ID gefunden: {client_id[:10]}...")
                return client_id

        # Fallback Client ID (falls scraping fehlschlägt)
        print("Verwende Fallback Client ID")
        return "xUA05eJLevezZTdSblP1r95c0s45DDtb"
...
def get_beatport_token():
    ...
    try:
        ...
        session_key = login_response.json().get("session_key")

        # Schritt 2: Authorization Code holen
        auth_params = {
            "client_id": client_id,
            "redirect_uri": "https://embed.beatport.com/callback",
            "response_type": "code",
